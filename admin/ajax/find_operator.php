...
if($product == 'glambar' || $product == 'GLAMBAR')
{
			...
			$sql_ad="select * from gamebardb_vodafone_qatar_report.operatorurls where product='glambar' order by operatorname asc ";
...
else if($product == '11Players' || $product == '11Players')
{
			...
			$sql_ad="select * from gamebardb_vodafone_qatar_report.operatorurls where product='11Players' order by operatorname asc ";
			$res_ad=mysqli_query($con,$sql_ad);
...
else if($product == 'Contest' || $product == 'contest')
{
			...
			$sql_ad="select * from gamebardb_vodafone_qatar_report.operatorurls where product='Contest' order by operatorname asc ";
			$res_ad=mysqli_query($con,$sql_ad);
...
else
{
	...
	$sql_ad="select * from gamebardb_vodafone_qatar_report.operatorurls where product='gamebar' order by operatorname asc";
		$res_ad=mysqli_query($con,$sql_ad);
...
?>
	...
	<select name="operator" id="operator" class="form-control select1_multiple"  onchange="myfun1()"required >
		...
		while($row_ad=mysqli_fetch_array($res_ad))
		{
			//echo $row_ad[0];exit;
		?>
		<option value="<?php echo $row_ad['operatorname']; ?>"><?php echo $row_ad['operatorname']; ?></option>
		<?php
		}
		?>
